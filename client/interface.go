...
type Client interface {
	// WriteData는 InputDataObj slice의 데이터를 write하고 그 결과를 tendermint의 ResultBroadcastTx로 return.
	...
	// ResultABCIQuery.Response.Value에 실제 read한 데이터가 OutputMetaDataObj의 slice로 담겨있음.
	Query(start uint64, end uint64, ownerKey []byte, qualifier []byte) (*ctypes.ResultABCIQuery, error)

	// Fetch는 InputQueryObj와 일치하는 데이터를 tendermint의 ResultABCIQuery에 담아서 return.
	// ResultABCIQuery.Response.Value에 실제 read한 데이터가 OutputRealDataObj의 slice로 담겨있음.
