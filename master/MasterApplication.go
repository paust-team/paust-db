...
func NewMasterApplication(serial bool, dir string) *MasterApplication {
	hash := make([]byte, 8)
	database, err := db.NewCRocksDB(types.DBName, dir)
...
func (app *MasterApplication) DeliverTx(tx []byte) abciTypes.ResponseDeliverTx {
	...
	for i := 0; i < len(wRealDataObjs); i++ {
		...
		app.mwb.SetColumnFamily(app.db.ColumnFamilyHandle(types.MetaCFNum), rowKey, metaByte)
		app.wb.SetColumnFamily(app.db.ColumnFamilyHandle(types.RealCFNum), rowKey, wRealDataObjs[i].Data)
	}
...
func (app *MasterApplication) metaDataQuery(query types.RMetaDataQueryObj) (types.RMetaDataResObjs, error) {
	...
	itr := app.db.IteratorColumnFamily(startByte, endByte, app.db.ColumnFamilyHandle(types.MetaCFNum))
...
func (app *MasterApplication) realDataQuery(query types.RRealDataQueryObj) (types.RRealDataResObjs, error) {
	...
	for _, rowKey := range query.Keys {
		valueSlice, err := app.db.GetDataFromColumnFamily(types.RealCFNum, rowKey)
		if err != nil {
			fmt.Print(err)
		}
